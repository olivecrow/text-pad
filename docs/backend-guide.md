...
- `supported-text-formats.json`: 프론트엔드와 Rust가 함께 읽는 제품 지원 형식·확장자·관례적 파일명·경로 패턴·샘플 목록.
...
- `open_file_paths(paths: Vec<String>) -> Result<Vec<OpenedFile>, FileCommandError>`
  - 메인 창에 드롭된 경로 중 실제 파일만 지원 인코딩의 크기·줄 수 제한 안에서 읽는다.
...
- `write_file_content(path: String, content: String, encoding: TextEncoding) -> Result<(), FileCommandError>`
  - 앞선 열기나 저장 대화상자에서 승인한 정규화 경로만 저장한다.
  - 탭에 기록된 인코딩과 BOM 유무를 유지해 같은 폴더의 임시 파일에 먼저 쓴 뒤 대상 파일로 교체한다.
- `save_file_dialog(default_name: String, content: String, encoding: Option<TextEncoding>, filters: Vec<DialogFilter>) -> Result<Option<SavedFile>, FileCommandError>`
  - Rust가 저장 경로를 직접 선택받아 저장하고, 성공한 경로와 실제 인코딩을 반환하며 경로를 승인 목록에 추가한다.
