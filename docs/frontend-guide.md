...
탭 끌기는 포인터 캡처(마우스 버튼을 놓을 때까지 시작 탭이 이동 입력을 계속 받는 방식)로 추적한다. 끌기를 시작하면 실제 탭 폭과 포인터를 잡은 위치를 유지한 탭 미리보기가 포인터를 따라가며, 창 간 이동 때도 포인터가 들어온 편집기 창에서 같은 미리보기를 이어서 표시한다. 창 간 이동 좌표는 모든 편집기 창에 알리되, 탭 원문 전달과 수신 확인은 지정한 편집기 창의 수신기만 처리해 원본 창에 복사본이 남지 않게 한다.
