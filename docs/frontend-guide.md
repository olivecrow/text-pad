...
- 편집 영역에서 탭 키를 누르면 실제 탭 문자가 아니라 공백 4개를 입력한다. 여러 줄이 선택되어 있으면 선택된 각 줄 앞에 공백 4개를 붙이고, `Shift+Tab`은 줄 앞의 공백 최대 4개나 탭 문자 1개를 제거한다.
- 줄 앞 들여쓰기 공백 안에 커서가 있으면 백스페이스 한 번으로 이전 4칸 경계까지 공백을 제거한다. 일반 텍스트 뒤의 백스페이스 동작은 브라우저 기본 동작을 유지한다.
- 사용자가 텍스트를 작성하거나 지우는 동안에는 캐럿을 깜빡이지 않는 고정 표시로 보여준다.
...
- 줄 번호.
- 들여쓰기 가이드.
- 닫힘이 있는 백틱 코드, 문자열, 숫자, 색상 코드, 데이터 파일 형식의 불리언 값, 파일 형식별 주석 강조.
