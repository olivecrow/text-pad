...
- 메인 창을 표시하고 편집기에 초점을 준 뒤 1초 후 앱 시작 세션당 한 번 업데이트를 조용히 확인한다.
- 시작 확인에서 새 버전을 찾으면 확인창을 띄우지 않고 메뉴바 오른쪽의 테마, 렌더 모드, 설정 버튼 왼쪽에 버전이 포함된 `업데이트` 버튼을 표시한다.
- 툴바의 `업데이트` 버튼을 누르면 별도의 업데이트 확인창 없이 기존 저장 확인을 거쳐 다운로드와 설치를 시작한다.
- `도움말(H) > 업데이트 확인`을 사용자가 직접 실행했을 때만 현재 버전, 새 버전, 릴리스 내용을 보여주고 설치 여부를 묻는다.
- 도움말 메뉴에서 `나중에`를 선택하거나 저장 확인을 취소해도 툴바의 `업데이트` 버튼은 유지한다.
- 수동 확인에서 새 버전이 없으면 하단 상태 표시줄에 현재 버전이 최신이라고 잠시 표시한다.
- 시작 확인 실패는 편집을 방해하지 않도록 알림 없이 넘긴다. 수동 확인이나 툴바에서 시작한 다운로드·설치에 실패하면 하단 상태 표시줄에 오류를 표시한다.
...
- `업데이트 확인`: 확인 중이거나 설치 중이면 비활성화한다. 시작 확인에서 이미 찾은 업데이트가 있으면 그 결과를 사용해 설치 확인창을 연다.
